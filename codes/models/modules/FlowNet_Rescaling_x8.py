...
class FlowNet(nn.Module):
                ...
                if level == 0:
                    ...
                    self.level0_condFlow = ConditionalFlow(num_channels=self.C,
                                                    ...
                                                    opt=opt['network_G']['flowDownsampler']['splitOff'],
                                                          num_levels_condition=2, SR=False)
                elif level == 1:
                    self.layers.append(Basic.Split(num_channels_split=self.C // 2 if level < self.L-1 else 3, level=level))
    ...
    def normal_flow(self, z, u=None, logdet=None, training=True):
        for layer, shape in zip(self.layers, self.output_shapes):
            if isinstance(layer, FlowStep):
                ...
                if layer.level == 0:
                    ...
                    fake_z3, conditional_feature3 = self.level2_condFlow(a3, z, logdet=logdet, reverse=False, training=training)

                    conditional_feature2 = torch.cat([y2, F.interpolate(conditional_feature3, scale_factor=2, mode='nearest')],1)
                    fake_z2, _ = self.level1_condFlow(a2, conditional_feature2, logdet=logdet, reverse=False, training=training)
        ...
        return z, fake_z1, fake_z2, fake_z3
    ...
    def reverse_flow(self, z, u=None, eps_std=None, training=True):
        for layer, shape in zip(reversed(self.layers), reversed(self.output_shapes)):
            if isinstance(layer, FlowStep):
                ...
                if layer.level == 2:
                    a3, conditional_feature3 = self.level2_condFlow(None, z, eps_std=eps_std, reverse=True, training=training)
                    ...
                    a2, conditional_feature2 = self.level1_condFlow(None, conditional_feature2, eps_std=eps_std, reverse=True, training=training)
