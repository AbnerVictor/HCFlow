    ...
    def profile_module_time(self, module, mode, *args, **kwargs):
        ...
        if mode == 'torchprofile':
            ...
            for i in range(5):
                ...
                torch.cuda.synchronize()
                print('Warm up iter {}: {}ms'.format(i, (time.time()-start)/1000))

            torch.backends.cudnn.deterministic = True
            torch.backends.cudnn.benchmark = True
            with torch.autograd.profiler.profile(use_cuda=True, record_shapes=True, profile_memory=False) as prof:
                out = module(*args, **kwargs)
            print(prof)
            # export trace
            prof.export_chrome_trace(self.path+'.json')
